...
" syn match       redFloat    "[+-]\=\(\d\+\('\d*\)*\)\=[,.]\d\=\(e[+-]\=\d\+\)\="
" syn match       redFloat    "[+-]\=\d\+\('\d*\)*\(e[+-]\=\d\+\)\="
...
syn keyword redPredefined about absolute action action? add ajoin all also alter and~ any any-block? any-function? any-object? any-path? any-string? any-word? append apply aqua arccosine arcsine arctangent as-pair ascii? ask assert at attempt
...
syn keyword redPredefined call case catch cause-error cd change change-dir changes char? charset chat check checksum clean-path clear clos close closure closure? coal coffee collect collect-words command? comment complement compose confirm construct context continue copy cosine cr create crimson crlf cyan
syn keyword redPredefined datatype? datatypes date? debase decimal? decloak decode decode-url default dehex delect delete delete-dir deline delta-profile delta-time demo difference dir? dirize divide do do-callback do-codec do-commands docs does dp ds dt dump dump-obj
syn keyword redPredefined echo eighth either email? empty? enbase encloak encode encoding? enline equal? equiv? error? escape even? event? evoke exclude exists? exit exp extend extract
syn keyword redPredefined fifth file-type? file? find find-all find-script first first+ forall foreach forest forever form format found? fourth frame? func funco function function?
...
syn keyword redPredefined if image? import in in-dir index? input insert integer? intern intersect invalid-utf? issue? ivory
...
syn keyword redPredefined magenta make make-banner make-dir map map-each map-event map-gob-offset map? maroon max maximum maximum-of min minimum minimum-of mint mkdir mod modify module module? modulo mold mold64 more move multiply
...
RebHi redComment Comment
RebHi redSheBang Comment
RebHi redOperator Operator
RebHi redLocal Special
RebHi redRefinementWord Constant
RebHi redRefinement Constant
RebHi redSpecial Special
RebHi redLitWord Constant
RebHi redLit Constant
RebHi redPredefined Keyword
RebHi redInteger Number
RebHi redFloat Number
RebHi redTuple Number
RebHi redSpecialCharacter Special
RebHi redString String
RebHi redType Type
RebHi redGet Identifier
RebHi redGetWord Identifier
RebHi redSet Identifier
